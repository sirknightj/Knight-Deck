...
//        }
        ...
        while(player.getActionPoints() > 0 && actionDeck.size() > 0 && !isBattleOver()) {
            ...
            CardFactory.getAttackCard(response).play(target);
            actionDeck.remove(CardFactory.getAttackCard(response));
            player.setActionPoints(player.getActionPoints() - CardFactory.getAttackCard(response).getCost());
            discardPile.add(CardFactory.getAttackCard(response));
            cardNames.remove(response);
    ...
    /**
     * Checks if the draw pile is empty. If so, refills the cards from the discard pile
     * after shuffling.
     */
    ...
    /**
     * Draws a specified number of cards from the draw pile and puts them in the player's hand.
     */
    ...
    /**
     * Checks if an enemy is dead and removes them from the battlefield.
     */
    ...
    /**
     * Looks inside each enemy's deck and chooses a random card to play. Each card in the
     * enemy's deck has an equal chance of appearing.
     */
